...
@section('breadcrumb_title', 'Profile User')
@section('content')
    <div class="col-lg-12">
        <div class="row">
            <div class="col-12">
                <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="tpd-setting-box profile">
                        <div class="tp-dashboard-banner-bg profile mb-100" style="margin-bottom: 100px"
                        ...
                        ">
                            <div class="tp-instructor-wrap d-flex justify-content-between">
                                ...
                                <div class="tp-instructor-course-btn profile">
                                    <a class="tp-btn-add-course profile profile_cover_photo_upload" href="#">
                                        <span>
                        ...
                        <div class="tpd-setting-from mt-5">
                            <div class="row">

                                <x-user::profile-input-text label="Full Name" name="name" value="{{ $user->name }}" />
                                <x-user::profile-input-text label="User Name" name="username"
                                    value="{{ $user->username }}" />
                                <x-user::profile-input-text label="Phone Number" name="phone"
                                    value="{{ $user->getMeta('phone') }}" placeholder="(+44) 433 962 004" />
                                <x-user::profile-input-text label="{{ __('Skill/Occupation') }}" name="skill"
                                    value="{{ $user->getMeta('skill') }}" />

                                <x-user::profile-input-textarea label="{{ __('Biography') }}" name="bio"
                                    value="{{ $user->getMeta('bio') }}" col=12 />
                            </div>
                        </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            var img = $('.tp-dashboard-banner-bg').data('background');
            $('.tp-dashboard-banner-bg').css('background-image', 'url(' + img + ')');


            $('.avater_upload').on('click', function() {
                $('#avatar').click();
            });
            ...
            $('#avatar').on('change', function() {
                ...
                reader.onload = function(e) {
                    $('.avatar').html(`<img src="${e.target.result}" alt="">`);
                }
                reader.readAsDataURL(file);
            });

            $('.profile_cover_photo_upload').on('click', function() {
                $('#cover_photo').click();
            });
            ...
            $('#cover_photo').on('change', function() {
                ...
                const reader = new FileReader();
                reader.onload = function(e) {
                    $('.tp-dashboard-banner-bg').css('background-image', `url(${e.target.result})`);
                }
                reader.readAsDataURL(file);
            });
        });
